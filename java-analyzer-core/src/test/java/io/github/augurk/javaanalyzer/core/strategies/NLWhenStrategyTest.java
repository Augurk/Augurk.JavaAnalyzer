...
public class NLWhenStrategyTest {
    private WhenStepStrategy target;

    @Before
    public void beforeEach() {
        target = new NLWhenStepStrategy();
    }

    @Test
    public void isWhenStep_ShouldReturnTrueWhenExpressionMatches() {
        assertThat(target.getLanguageCode(), equalTo("nl"));
        assertThat(target.isWhenStep("als"), is(true));
        assertThat(target.isWhenStep("wanneer"), is(true));
    }

    @Test
    public void isWhenStep_ShouldReturnFalseWhenExpressionNotMatches() {
        assertThat(target.isWhenStep("invalid"), is(false));
        assertThat(target.isWhenStep("when"), is(false));
    }
}
